...
__STATIC_INLINE uint32_t __NVIC_GetVector(IRQn_Type IRQn) {
  uint32_t vectors = (uint32_t)SCB->VTOR;
  return (
      uint32_t)(*(int *)(vectors + ((int32_t)IRQn + NVIC_USER_IRQ_OFFSET) * 4));
}
...
#define __SCB_DCACHE_LINE_SIZE                                                 \
  32U /*!< Cortex-M7 cache line size is fixed to 32 bytes (8 words). See also  \
         register SCB_CCSIDR */
...
__STATIC_FORCEINLINE void SCB_InvalidateICache_by_Addr(void *addr,
  ...
  if (isize > 0) {
    ...
    do {
      SCB->ICIMVAU = op_addr; /* register accepts only 32byte aligned values,
                                 only bits 31..5 are valid */
...
__STATIC_FORCEINLINE void SCB_InvalidateDCache_by_Addr(void *addr,
  ...
  if (dsize > 0) {
    ...
    do {
      SCB->DCIMVAC = op_addr; /* register accepts only 32byte aligned values,
                                 only bits 31..5 are valid */
...
__STATIC_FORCEINLINE void SCB_CleanDCache_by_Addr(uint32_t *addr,
  ...
  if (dsize > 0) {
    ...
    do {
      SCB->DCCMVAC = op_addr; /* register accepts only 32byte aligned values,
                                 only bits 31..5 are valid */
      op_addr += __SCB_DCACHE_LINE_SIZE;
      op_size -= __SCB_DCACHE_LINE_SIZE;
    } while (op_size > 0);
...
__STATIC_FORCEINLINE void SCB_CleanInvalidateDCache_by_Addr(uint32_t *addr,
  ...
  if (dsize > 0) {
    ...
    do {
      SCB->DCCIMVAC = op_addr; /* register accepts only 32byte aligned values,
                                  only bits 31..5 are valid */
      op_addr += __SCB_DCACHE_LINE_SIZE;
      op_size -= __SCB_DCACHE_LINE_SIZE;
    } while (op_size > 0);
