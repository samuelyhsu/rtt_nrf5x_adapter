...
static struct nrfx_drv_spi
    spi_bus_obj[sizeof(spi_config) / sizeof(spi_config[0])];
...
nrfx_spim_evt_handler_t spi_handler[] = {
    spi0_handler, spi1_handler, spi2_handler, spi3_handler, spi4_handler};
...
static rt_err_t spi_configure(struct rt_spi_device *device,
  ...
  nrfx_spim_config_t config = NRFX_SPIM_DEFAULT_CONFIG(
      bsp_spi_pin[index].sck_pin, bsp_spi_pin[index].mosi_pin,
      bsp_spi_pin[index].miso_pin, NRFX_SPIM_PIN_NOT_USED);
  ...
  rt_memcpy((void *)&spi_bus_obj[index].spi_config, (void *)&config,
            sizeof(nrfx_spim_config_t));
  nrfx_spim_evt_handler_t handler =
      RT_NULL; // spi send callback handler ,default NULL
...
static rt_uint32_t spixfer(struct rt_spi_device *device,
...
static int rt_hw_spi_bus_init(void) {
  ...
  for (int i = 0; i < sizeof(spi_config) / sizeof(spi_config[0]); i++) {
    ...
    result = rt_spi_bus_register(&spi_bus_obj[i].spi_bus,
                                 spi_config[i].bus_name, &nrfx_spi_ops);
    RT_ASSERT(result == RT_EOK);
  }
  return result;
}

int rt_hw_spi_init(void) { return rt_hw_spi_bus_init(); }
...
rt_err_t rt_hw_spi_device_attach(const char *bus_name, const char *device_name,
  ...
  result = rt_spi_bus_attach_device(spi_device, device_name, bus_name,
                                    (void *)ss_pin);
  if (result != RT_EOK) {
...

{
  ...
  if (!spi_dev) {
    ...
    while (1) {
      rt_spi_send(spi_dev, m_tx_buf, 6);
...
nrfx_spi_evt_handler_t spi_handler[] = {spi0_handler, spi1_handler,
                                        spi2_handler};
...
static rt_err_t spi_configure(struct rt_spi_device *device,
  ...
  nrfx_spi_config_t config = NRFX_SPI_DEFAULT_CONFIG(
      bsp_spi_pin[index].sck_pin, bsp_spi_pin[index].mosi_pin,
      bsp_spi_pin[index].miso_pin, NRFX_SPI_PIN_NOT_USED);
