...
#define RTC_PRESCALER                                                          \
  ((uint32_t)(NRFX_ROUNDED_DIV(SYSTICK_CLOCK_HZ, TICK_FREQUENCE_HZ) - 1))
...
const static struct rt_device_ops rtc_ops = {RT_NULL, RT_NULL, RT_NULL,
                                             RT_NULL, RT_NULL, rt_rtc_control};
#endif

#if defined(SOC_NRF5340)
static rt_err_t rt_hw_rtc_register_5340(rt_device_t device, const char *name,
                                        rt_uint32_t flag) {
