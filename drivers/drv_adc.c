...
/* channel: 0-7 */
static rt_err_t nrf5x_adc_enabled(struct rt_adc_device *device,
  ...
  if (enabled) {
    ...
    if (results.channel_count == 0) {
      ...
      err_code =
          nrfx_saadc_buffer_set(result_buff_cache, results.channel_count);
    }
  }
...
static rt_err_t nrf5x_get_adc_value(struct rt_adc_device *device,
...
int rt_hw_adc_init(void) {
  ...
  if (nrfx_saadc_init(NRFX_SAADC_CONFIG_IRQ_PRIORITY) != NRFX_SUCCESS) {
    ...
    /* register ADC device */
    if (rt_hw_adc_register(&nrf5x_adc_device, name_buf, &nrf5x_adc_ops,
                           nrf5x_adc_device.parent.user_data) == RT_EOK) {
...
void saadc_sample(void) {
  ...
  channel_config =
      (drv_nrfx_saadc_channel_t){.mode = SAMPLE_ADC_MODE_SINGLE_ENDED,
