...
#define PWM0_CONFIG                                                            \
  { .pwm_handle = &m_pwm0, .name = "pwm0", .pwm_src_clk = 1000000, }
#endif

#ifdef BSP_USING_PWM1
static nrfx_pwm_t m_pwm1 = NRFX_PWM_INSTANCE(1);
#define PWM1_CONFIG                                                            \
  { .pwm_handle = &m_pwm1, .name = "pwm1", .pwm_src_clk = 1000000, }
#endif

#ifdef BSP_USING_PWM2
static nrfx_pwm_t m_pwm2 = NRFX_PWM_INSTANCE(2);
#define PWM2_CONFIG                                                            \
  { .pwm_handle = &m_pwm2, .name = "pwm2", .pwm_src_clk = 1000000, }
#endif

#ifdef BSP_USING_PWM3
static nrfx_pwm_t m_pwm3 = NRFX_PWM_INSTANCE(3);
#define PWM3_CONFIG                                                            \
  { .pwm_handle = &m_pwm3, .name = "pwm3", .pwm_src_clk = 1000000, }
#endif
...
static rt_err_t drv_pwm_enable(struct mcu_pwm *p_mcu,
                               struct rt_pwm_configuration *configuration,
                               rt_bool_t enable) {
  if (!enable) {
    nrfx_pwm_stop(p_mcu->pwm_handle, true);
  } else {
    (void)nrfx_pwm_simple_playback(p_mcu->pwm_handle, &p_mcu->m_demo1_seq, 1,
                                   NRFX_PWM_FLAG_LOOP);
  }
...
static rt_err_t drv_pwm_get(struct mcu_pwm *pwm_handle,
                            struct rt_pwm_configuration *configuration) {
  ...
  configuration->period =
      pwm_handle->pwm_handle->p_registers->COUNTERTOP * 1000UL / tick_pscond;
  configuration->pulse =
      pwm_handle->pwm_handle->p_registers->SEQ[channel_number].PTR /
      tick_pscond;
...
static rt_err_t drv_pwm_set(struct mcu_pwm *p_mcu,
                            struct rt_pwm_configuration *configuration) {
  rt_uint32_t period, pulse;
  uint8_t tick_pscond;
  tick_pscond = p_mcu->pwm_src_clk / 1000000UL;

  p_mcu->pwm_handle->p_registers->COUNTERTOP =
      (unsigned long long)configuration->period * tick_pscond;
...
#define _PRIO_APP_LOWEST 7
  nrfx_pwm_config_t config0 = {.irq_priority = _PRIO_APP_LOWEST,
  ...
  device->m_demo1_seq.length =
      NRF_PWM_VALUES_LENGTH(device->m_demo1_seq_values),
...
static int mcu_pwm_init(void) {
  ...
  for (i = 0; i < sizeof(mcu_pwm_obj) / sizeof(mcu_pwm_obj[0]); i++) {
    ...
    if (mcu_hw_pwm_init(&mcu_pwm_obj[i]) != RT_EOK) {
      ...
      /* register pwm device */
      if (rt_device_pwm_register(&mcu_pwm_obj[i].pwm_device,
                                 mcu_pwm_obj[i].name, &drv_ops,
                                 &mcu_pwm_obj[i].pwm_handle) == RT_EOK) {
