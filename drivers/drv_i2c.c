...
#include <rtdevice.h>
    ...
    defined(BSP_USING_I2C2) || defined(BSP_USING_I2C3)
typedef struct {
...
static drv_i2c_cfg_t drv_i2c_0 = {.freq = NRF_TWIM_FREQ_400K,
...
static drv_i2c_cfg_t drv_i2c_1 = {.freq = NRF_TWIM_FREQ_400K,
...
static drv_i2c_cfg_t drv_i2c_2 = {.freq = NRF_TWIM_FREQ_400K,
...
static drv_i2c_cfg_t drv_i2c_3 = {.freq = NRF_TWIM_FREQ_400K,
...
static rt_ssize_t _master_xfer(struct rt_i2c_bus_device *bus,
  ...
  for (i = 0; i < num; i++) {
    ...
    nrfx_twim_xfer_desc_t xfer =
        NRFX_TWIM_XFER_DESC_TX(msg->addr, msg->buf, msg->len);
