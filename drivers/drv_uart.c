...
static void uart0_event_hander(nrfx_uart_event_t const *p_event,
                               void *p_context);
drv_uart_cfg_t m_uart0_cfg = {.uart = NRFX_UART_INSTANCE(0),
                              .rx_byte = 0,
                              .rx_length = 0,
...
static rt_err_t _uart_cfg(struct rt_serial_device *serial,
  ...
  nrfx_uart_config_t config =
      NRFX_UART_DEFAULT_CONFIG(BSP_UART0_TX_PIN, BSP_UART0_RX_PIN);
...
rt_weak int uart_putc_hook(rt_uint8_t *ch) { return -1; }
...
rt_weak int uart_getc_hook(rt_uint8_t *ch) { return -1; };
