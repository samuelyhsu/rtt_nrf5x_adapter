...
#define TIM_DEV_INFO_CONFIG                                                    \
  {                                                                            \
    .maxfreq = 16000000, .minfreq = 31250, .maxcnt = 0xFFFFFFFF,               \
...
static void timer_callback(nrf_timer_event_t event_type, void *p_context) {
  ...
  // no matter what event_type is(NRF_TIMER_EVENT_COMPARE0 or others), call same
  // function "rt_device_hwtimer_isr"
  LOG_D("timer_callback event_type = %d, inst_id = %d, cc conunt = %d\r\n",
        event_type, timer_info->timer_inst.instance_id,
        timer_info->timer_inst.cc_channel_count);
...
static rt_err_t timer_start(rt_hwtimer_t *timer, rt_uint32_t t,
  ...
  if (opmode == HWTIMER_MODE_ONESHOT) {
    // means TIMER_SHORTS_COMPARE0_STOP_Msk or TIMER_SHORTS_COMPARE1_STOP_Msk
    // ..., according to cc_channel.
    mask = (nrf_timer_short_mask_t)(1 << (timer_info->cc_channel + 8));
  } else {
    // means TIMER_SHORTS_COMPARE0_CLEAR_Msk or TIMER_SHORTS_COMPARE1_CLEAR_Msk
    // ..., according to cc_channel.
    mask = (nrf_timer_short_mask_t)(1 << timer_info->cc_channel);
  }

  nrfx_timer_extended_compare(&(timer_info->timer_inst), timer_info->cc_channel,
                              t, mask, true);
...
static int nrf5x_hwtimer_init(void) {
  ...
  for (i = 0; i < sizeof(nrf5x_hwtimer_obj) / sizeof(nrf5x_hwtimer_obj[0]);
       i++) {
    nrf5x_hwtimer_obj[i].timer_device.info = &_info;
    nrf5x_hwtimer_obj[i].timer_device.ops = &_ops;
    if (rt_device_hwtimer_register(
            &nrf5x_hwtimer_obj[i].timer_device, nrf5x_hwtimer_obj[i].name,
            &nrf5x_hwtimer_obj[i].timer_info) == RT_EOK) {
